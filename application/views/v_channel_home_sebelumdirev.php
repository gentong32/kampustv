...
<div style="text-align: center;background-color:#8cbdac;padding-top: 20px;padding-bottom: 20px">
    ...
    <div class="regular slider" style="margin-top: 20px;">
		...
		<div style="margin-top:20px;font-weight: bold;color: #e8df55"><button class="myButtongreen" onclick="window.open('<?php
			echo base_url();?>channel/sekolah/','_self')">LIHAT SEMUA CHANNEL</button></div>
   ...
   // console.log("Gabung:" + (gabung));
    ...
    function ceklive() {
        ...
        if (tglsaiki - tgljadwal < 0) {
			...
			<?php if ($jmldaf_list > 0) {?>
            $('#layartancap').show();
            $('#keteranganLive').html("SEGERA TAYANG TANGGAL: <?php if ($punyalist) {
                if ($id_playlist == null)
    ...
    */
	...
	while (batasmalam==false) {
		...
		for (var y = 1; y <= jmlterm; y++) {
			for (var z = 1; z <= jmljudul; z++) {
				...
				detikkedepan = keJam(new Date("1970-01-01T" + detikke[z+1]).getTime() + durasi * y);

				//console.log("detikke"+(y * jmljudul + z)+":"+detikke[y * jmljudul + z]);
				waktuakhir = new Date(now.getFullYear(), now.getMonth(), now.getDate(), '23', '59', '59');
				waktujatah = new Date(now.getFullYear(), now.getMonth(), now.getDate(),
					detikke[y * jmljudul + z].substring(0,2), detikke[y * jmljudul + z].substring(3,5),
					detikke[y * jmljudul + z].substring(6,8));
				waktujatahdepan = new Date(now.getFullYear(), now.getMonth(), now.getDate(),
					detikkedepan.substring(0,2), detikkedepan.substring(3,5),
					detikkedepan.substring(6,8));

				if (waktujatahdepan.getTime()<waktujatah.getTime())
				{
					termakhir = jmlterm;
					zakhir = z;
					batasmalam = true;
				...
				if (waktujatah.getTime()>waktuakhir.getTime()) {
					batasmalam = true;
					termakhir = jmlterm;
					zakhir = z;
					break;
				}
				//console.log(waktujatah.getTime());
    ...
    function updateTanggal() {
...

        {
            ...
            //updatePlaying();
            for (a = 1; a <= (jmljudul * termakhir + zakhir); a++) {
            ...
            if (cekjatah != jatah) {
                ...
                if (detiklokal > durasike[jatah]) {
					...
					{
						console.log("onload");
						player.loadVideoById(idvideo[jatah]);
					}
                } else {
					if (readyyoutube)
					{
                ...
                if (statusPlayer != 2 && yoberangkat==true) {
					//console.log("MAIN");
                    if (jarak > 5 || jarak < -5)
					{
						if (readyyoutube)
